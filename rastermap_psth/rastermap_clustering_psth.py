...
def table1b_trial_counts_per_mouse(event_map, cfg, out_dir):
    """Save per-mouse trial counts (global, odd, even) as a CSV table."""
    mice      = sorted({key[0] for key in event_map})
    mouse_idx = {m: i for i, m in enumerate(mice)}
    n_mice, n_conds = len(mice), len(CONDITIONS)

    global_counts = np.zeros((n_mice, n_conds), dtype=int)
    odd_counts    = np.zeros((n_mice, n_conds), dtype=int)
    even_counts   = np.zeros((n_mice, n_conds), dtype=int)

    for (mouse_id, session_id, context, trial_type, align_col), events in event_map.items():
        mi = mouse_idx.get(mouse_id)
        if mi is None:
            continue
        for ci, ((tt, ctx), acol) in enumerate(zip(CONDITIONS, COND_ALIGN_COLS)):
            if tt == trial_type and ctx == context and acol == align_col:
                global_counts[mi, ci] += len(events)
                odd_counts[mi, ci]    += len(events[0::2])
                even_counts[mi, ci]   += len(events[1::2])

    rows = []
    for mi, mouse_id in enumerate(mice):
        for ci, label in enumerate(COND_LABELS):
            rows.append({
                "mouse_id":    mouse_id,
                "condition":   label,
                "n_global":    global_counts[mi, ci],
                "n_odd":       odd_counts[mi, ci],
                "n_even":      even_counts[mi, ci],
            })

    df = pd.DataFrame(rows)
    csv_path = out_dir / "table1b_trial_counts_per_mouse.csv"
    df.to_csv(csv_path, index=False)
    print(f"  saved table1b_trial_counts_per_mouse ({len(mice)} mice × {n_conds} conditions)")
    return df
...
def fig8_umap_old(emb, cluster_labels, n_clusters, out_dir):
    fig, (ax1, ax2, cax) = plt.subplots(1, 3, figsize=(11, 5),gridspec_kw={"width_ratios": [1, 1, 0.05]})
    ax1.scatter(emb[:, 0], emb[:, 1], s=1, c="steelblue", alpha=0.6, linewidths=0)
    ax1.set_title("UMAP"); ax1.set_xlabel("UMAP 1"); ax1.set_ylabel("UMAP 2")
    sc = ax2.scatter(emb[:, 0], emb[:, 1], s=1, c=cluster_labels,
                     cmap="turbo", alpha=0.6, linewidths=0, vmin=0, vmax=n_clusters - 1)
    ax2.set_title("UMAP — rastermap_psth clusters"); ax2.set_xlabel("UMAP 1"); ax2.set_ylabel("UMAP 2")
...
def fig8_umap(emb, cluster_labels, n_clusters, out_dir, prefix=None):
    ...
    sc = ax2.scatter(emb[:, 0], emb[:, 1],s=1,c=cluster_labels,cmap="turbo",alpha=0.6,linewidths=0,vmin=0,vmax=vmax,)
...
def run_rastermap_psth(units: pd.DataFrame,
    ...
    bl_tag               = cfg["baseline_removal"]
    bl_tag_txt = 'no_baseline' if bl_tag else 'baseline'
    data_folder = Path(out_root, "rastermap_psth_jaw_test_test_anat", "input_data")
    data_folder.mkdir(parents=True, exist_ok=True)
    out_folder = Path(out_root, "rastermap_psth_jaw_test_test_anat", f"n_clusters_{n_rastermap_clusters}",
                      period_tag, norm_tag, mod_tag, reward_tag, bl_tag_txt)
    ...
    # Identify mice with too few trials, exclude them
    mice_too_few_trials = filtered_df['mouse_id'].unique()
    if len(mice_too_few_trials) > 0:
    ...
    # Anatomical axis scores (S1 axonal innervation, hierarchy scores) — optional.
    if cfg.get("anatomy_score_cols"):
        anatomy_maps = load_anatomy_scores(units_good, cfg["anatomy_score_cols"])
        axon_arr   = np.array([anatomy_maps["avg_ipsi"].get(uid, np.nan)
                                for uid in unit_ids], dtype=float)
        harris_arr = np.array([anatomy_maps["cc_tc_ct_iterated"].get(uid, np.nan)
                                for uid in unit_ids], dtype=float)
        gao_arr    = np.array([anatomy_maps["cc_hierarchy_score_columns"].get(uid, np.nan)
                                for uid in unit_ids], dtype=float)
    else:
    ...
    bl_tag = "nobl" if cfg.get("baseline_removal", True) else "bl"
    fname = f"feature_matrix_{bl_tag}_{cfg.get('normalize', 'zscore')}.npy"
    fpath = data_folder / fname

    # ── full-dataset ───────────────────────────────────────────────────
    if cfg.get("do_global", False):
        print("\nRunning full-dataset mode (all trials)...")

        if cfg.get("recompute_feature_matrix", True):
            ...
            if not fpath.exists():
                raise FileNotFoundError(
                    f"recompute_feature_matrix=False but no saved matrix found at:\n  {fpath}\n"
                    "Set recompute_feature_matrix=True to build it first.")
            print(f"Loading pre-computed feature matrix from {fpath.name} ...")
            X = np.load(fpath)
            # Reconstruct t_ctrs and n_bins_list from cond_infos (same cfg/conditions as
            # when the matrix was built — no spike data needed).
            t_ctrs = [info[2] for info in cond_infos]
            n_bins_list = [info[3] for info in cond_infos]
        ...
        if odd.sum() > 0:
            ...
            if axon_arr is not None:
                axon_arr   = axon_arr[~odd]
                harris_arr = harris_arr[~odd]
                gao_arr    = gao_arr[~odd]
        ...
        #fig6b_cluster_profiles_reward_groups(X, t_ctrs, n_bins_list, isort, boundaries, COND_LABELS, COND_COLORS, reward_arr, out_folder)
        ...
        if axon_arr is not None:
            ...
            fig13_anatomy_axis_per_cluster(cluster_labels, gao_arr,    "Hierarchy score\n(Gao '26)",
                                            "cc_hierarchy_score_columns",    n_k, out_folder)

    # ── cross-validation ───────────────────────────────────────────────────
    if cfg.get("cross_validate", False):
        print("\nRunning cross-validation mode (odd/even trial split)...")

        event_map_odd, event_map_even = split_event_map(event_map)

        print("  Building X_odd (fit normaliser)...")
        X_odd, t_ctrs, n_bins_list, norms_list = build_feature_matrix_strided(
        ...
        fig5_population_matrix(X_even, n_bins_list, isort_cv, boundaries_cv, vmax_cv, cfg,
                               ...
                               axon_arr=axon_arr, harris_arr=harris_arr, gao_arr=gao_arr,
                               anatomy_cmaps=anatomy_cmaps, area_arr=area_arr)
        fig6_cluster_profiles(X_even, t_ctrs, n_bins_list, isort, boundaries, out_folder, prefix="cv_even")
        fig6b_cluster_profiles_reward_groups(X_even, t_ctrs, n_bins_list, isort, boundaries,
                                       COND_LABELS, COND_COLORS, reward_arr, out_folder, prefix="cv_even")
        emb = umap.UMAP(n_neighbors=cfg["umap_n_neighbors"],
                        min_dist=cfg["umap_min_dist"],
                        n_components=2,
                        random_state=42).fit_transform(X_even)
        fig8_umap(emb, cluster_labels, n_k, out_folder, prefix="cv_even")
        ...
        save_dict = dict(
            X=X_even,
            isort=isort_cv,
            boundaries=boundaries_cv,
            cluster_labels=cluster_labels_cv,
            umap_embedding=emb,
            km_labels      = km_labels,
    ...
    # ── save per-neuron cluster/metadata table (join key for other analyses) ──
    cluster_id_map      = units_good["cluster_id"].to_dict()
    electrode_group_map = units_good["electrode_group"].to_dict()

    neuron_meta_df = pd.DataFrame({
        "unit_id":         unit_ids,
        "cluster_label":   cluster_labels,
        "km_label":        km_labels,
        "cluster_id":      [cluster_id_map.get(uid)      for uid in unit_ids],
        "mouse_id":        [mouse_map.get(uid)           for uid in unit_ids],
        "session_id":      [session_map.get(uid)         for uid in unit_ids],
        "electrode_group": [electrode_group_map.get(uid) for uid in unit_ids],
    ...
    if axon_arr is not None:
        neuron_meta_df["avg_ipsi"]      = axon_arr
        neuron_meta_df["cc_tc_ct_iterated"] = harris_arr
        neuron_meta_df["cc_hierarchy_score_columns"]    = gao_arr
    meta_path = out_folder / "neuron_cluster_labels.csv"
    neuron_meta_df.to_csv(meta_path, index=False)
    print(f"  Neuron cluster/metadata table saved → {meta_path.name}  "
          f"({len(neuron_meta_df)} neurons, one row each)")

    # ── save a SEPARATE per-neuron table for the cross-validated embedding ────
    # (same metadata, but cluster_label_cv from the odd-trial-only Rastermap fit
    # instead of the main cluster_label; only written when cross_validate=True)
    if cfg.get("cross_validate", False):
        neuron_meta_cv_df = pd.DataFrame({
            "unit_id":            unit_ids,
            "cluster_label_cv":   cluster_labels_cv,
            "km_label":           km_labels,
            "cluster_id":         [cluster_id_map.get(uid)      for uid in unit_ids],
            "mouse_id":           [mouse_map.get(uid)           for uid in unit_ids],
            "session_id":         [session_map.get(uid)         for uid in unit_ids],
        ...
        if axon_arr is not None:
            neuron_meta_cv_df["axonal_innervation_s1"]      = axon_arr
            neuron_meta_cv_df["hierarchy_score_harris2019"] = harris_arr
            neuron_meta_cv_df["hierarchy_score_gao2026"]    = gao_arr

        assert len(neuron_meta_cv_df) == len(unit_ids), (
            f"neuron_meta_cv_df has {len(neuron_meta_cv_df)} rows but there are "
            f"{len(unit_ids)} unit_ids — expected exactly one row per neuron.")

        meta_cv_path = out_folder / "neuron_cluster_labels_cv.csv"
        neuron_meta_cv_df.to_csv(meta_cv_path, index=False)
        print(f"  Neuron cluster/metadata table (cross-validated) saved → {meta_cv_path.name}  "
              f"({len(neuron_meta_cv_df)} neurons, one row each)")

    # ── save per-neuron PSTHs split by trial-type condition ───────────────
    def _sanitize_cond_label(label):
        return re.sub(r"[^0-9a-zA-Z]+", "_", label).strip("_").lower()

    psth_path = out_folder / "neuron_psth_by_condition.npz"
    psth_dict = dict(
        unit_ids    = np.array(unit_ids),
        cond_labels = np.array(COND_LABELS),
    )
    _col = 0
    for label, nb, t_ctr_c in zip(COND_LABELS, n_bins_list, t_ctrs):
        tag = _sanitize_cond_label(label)
        psth_dict[f"psth__{tag}"]  = X[:, _col:_col + nb]
        psth_dict[f"t_ctr__{tag}"] = t_ctr_c
        _col += nb
    np.savez_compressed(psth_path, **psth_dict)
    print(f"  Per-condition PSTHs saved → {psth_path.name}")
