...
def _neuron_vector_strided(st, mouse_id, session_id, event_map, cond_infos, cfg, conds,
    ...
    def _mean_psth(trial_type, context, t_pre, t_post, n_out, align_col, base_mask):
        ...
        # Per-trial baseline subtraction: remove each trial's mean pre-stimulus
        # activity before averaging, so trial-to-trial FR drift doesn't inflate
        # the mean PSTH baseline.
        rates = rates - rates[:, base_mask].mean(axis=1, keepdims=True) #TOOD: decide if kept?

        if cfg.get("square_fr", False):
            rates = np.sign(rates) * rates ** 2 #negative stays negative

        return rates.mean(axis=0)

    psths      = [_mean_psth(tt, ctx, t_pre, t_post, n_out, acol, base_mask)
                  for (tt, ctx), (t_pre, t_post, t_ctr_c, n_out, base_mask), acol
                  in zip(conds, cond_infos, cond_align_cols)]
...
def run_rastermap_psth(units: pd.DataFrame,
    ...
    out_folder = Path(out_root, "rastermap_psth_jaw", f"n_clusters_{n_rastermap_clusters}",
...
def _fig_cluster_stats_matrix(f_matrix, reward_groups, pvals_fdr, reject,
    ...
    im_mean = ax_mean.imshow(mean_mat[::-1], aspect="auto", interpolation="none",
    ...
    im = ax_diff.imshow(diff_mat[::-1], aspect="auto", interpolation="none",
