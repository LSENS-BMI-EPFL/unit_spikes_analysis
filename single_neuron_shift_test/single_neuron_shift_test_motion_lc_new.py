...
DEFAULT_CONFIG = {
    ...
    "n_bootstrap":       1000,       # for CI on forest-plot means
...
def _compute_partial_r_column(df: pd.DataFrame,
    ...
    for epoch, r_lm in r_lm_by_epoch.items():
        if not np.isfinite(r_lm):
            continue
        lc_sub  = df[lc_mask  & (df["epoch"] == epoch)].set_index("neuron_id")
        mot_sub = df[mot_mask & (df["epoch"] == epoch)].set_index("neuron_id")
...
def add_quadrant_column(df: pd.DataFrame, use_p: str) -> pd.DataFrame:
    ...
    for epoch in ["baseline", "evoked"]:
        ...
        if not mot_mask.any() or not lc_mask.any():
            continue
        # map significant flag from motion rows onto each lc row by neuron_id
        sig_motion_map = df.loc[mot_mask].set_index("neuron_id")["significant"]
        lc_idx = df.index[lc_mask]
        sig_m  = df.loc[lc_mask, "neuron_id"].map(sig_motion_map).fillna(False).astype(bool)
...
class NeuronTestResult:
    ...
    reward_group:   Optional[int] = None
    epoch:          str = ""
    factor:         str = ""
    n_trials:       int = 0
    r:              float = np.nan
    m:              int = 0
    p_conservative: float = np.nan
    p_approx:       float = np.nan
...
def _drift_test_seed(base_seed, session_id, neuron_id, epoch) -> np.random.SeedSequence:
    ...
    across neurons."""
    tag = abs(hash((str(session_id), str(neuron_id), epoch))) % (2**32)
...
def write_drift_test_warnings(results: pd.DataFrame, out_dir: Path,
    ...
    for _, row in flagged.iterrows():
        msg = (f"unit {row['neuron_id']}  epoch={row['epoch']}  "
...
def analyze_session(unit_table, trial_table, session_id, mouse_id,
    ...
    # join downstream (quadrant classification, partial_r, example-figure
    # lookups) silently misaligns or drops rows on a collision instead of
    # erroring, so a duplicate here would corrupt results without any
    # visible sign. cluster_id is NOT usable as this key on its own -- it
    # can repeat across different imec_id/electrode_group in the same file.
    ...
    for _, unit in units.iterrows():
        ...
        for ep, event_times, event_window, bin_width, n_shifts in [
            ...
            if motion_error or np.isnan(t_start) or np.isnan(t_end):
                ...
                seed = _drift_test_seed(base_seed, session_id, neuron_id, ep)
...
def plot_motion_alignment(unit_table, trial_table, session_id, mouse_id,
    ...
    for imec, (depth, mo) in probes_seen.items():
        ...
        n_show = min(len(all_st), 1000)        # cap at 300 trials for clarity
        tx     = all_st[:n_show]
...
def plot_single_neuron_examples(results, unit_table, trial_table,
    ...
    chosen_ids = []
    for q in QUAD_ORDER:
        pool = ev_lc[ev_lc["quadrant"] == q]["neuron_id"].unique()
    ...
    for ri, uid in enumerate(chosen_ids):
        ...
        row_ev_lc = results[(results["neuron_id"] == uid) &
                            (results["epoch"] == "evoked") &
                            (results["factor"] == "learning_curve")]
        row_bs_mo = results[(results["neuron_id"] == uid) &
...
def plot_grand_summary(combined, config, out_dir, trial_table):
    ...
    for bc in ["both", "good", "mua"]:
        ...
        lc_idx = ev_lc2.set_index("neuron_id")
        mo_idx = ev_mo.set_index("neuron_id")
...
def run_shift_test_analysis(unit_table, trial_table, output_path,
    ...
    required_unit  = {"session_id", "mouse_id", "cluster_id", "electrode_group", "neuron_id",
