...
def _permanova(vals_rp: np.ndarray, vals_rm: np.ndarray,
    ...
    perm = np.array([_f_stat(combined, rng.permutation(labels)) for _ in range(n_perm)])
...
def plot_lag_difference_distribution(lag_df: pd.DataFrame, direction: int,
    ...
    for ax, data, label in [(axes[0], sub["lag"].values, "Neuron level"),
...
def _build_pointplot(lag_df, ordered, direction, xlim, ax, annotate_sig=True):
    ...
    areas_tested = list(pvals.keys())
    if areas_tested and annotate_sig:
        _, pvals_corr, _, _ = multipletests(
            [pvals[a] for a in areas_tested], method="fdr_bh")
        for area, pc in zip(areas_tested, pvals_corr):
            if pc < 0.05:
...
def plot_lag_summary_pointplot(lag_df: pd.DataFrame, shared_areas: list[str],
    ...
    fig.tight_layout()
    _savefig(fig, out_dir / "lag_summary_pointplot_allen.png")
    plt.close(fig)
    ...
    fig.suptitle("Lag by area — mouse-median ± SEM  (ordered by |lag diff| R+−R-)",
                 fontsize=15, y=1.01)
    fig.tight_layout()
    _savefig(fig, out_dir / "lag_summary_pointplot_lagdiff_order.png")
    plt.close(fig)
...
def plot_lag_summary_heatmap(lag_df: pd.DataFrame, shared_areas: list[str],
    ...
    for direction in [1, -1]:
        ...
        for hax, max_, rg, mat, peaks in [
            ...
            hax.set_xlabel("Lag (trials)"); hax.set_xlim(lag_range)
        ...
        fig_k, axes_k = plt.subplots(1, 2,
                                      ...
                                      sharey=True)
        cmap_areas = plt.get_cmap("winter", len(peak_ordered))
...
def _make_summary_figures(results_df: pd.DataFrame, lag_df: pd.DataFrame):
    ...
    print(f"Shared areas (pointplot, ≥3 mice/group): {shared_areas}")
    ...
    plot_lag_distribution_by_group(lag_df, shared_areas, out_dir=FIG_OUT)
