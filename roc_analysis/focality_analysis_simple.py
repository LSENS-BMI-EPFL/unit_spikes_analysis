"""
Single function for focality index roc_analysis of GLM encoding variables.
...
def analyze_focality_old(data_df, saving_path=None, n_bootstrap=1000, n_permutations=1000):
    """
    Focality roc_analysis: resample neurons to get distributions, test difference.
...
def analyze_focality(data_df, saving_path=None, area_column='area_acronym_custom',
    ...
    """
    Complete focality roc_analysis using Approach 2: Group-level with mouse-resampling.
...
def analyze_focality_with_bca(data_df, saving_path=None, n_bootstrap=1000,
    ...
    """
    Focality roc_analysis with bias-corrected accelerated (BCa) bootstrap CIs.
...
def analyze_focality_neurons(data_df, saving_path=None, area_column='area_acronym_custom',
                             n_bootstrap=5000, n_permutations=10000):
    """
    Focality roc_analysis by resampling NEURONS (not mice).

    WARNING: This approach ignores hierarchical structure (neurons nested in mice).
    Use for exploratory roc_analysis only. For rigorous inference, use mouse-level resampling.
    ...
    print("⚠️  Use for exploratory roc_analysis only")
    ...
    for model in results_df['model_name'].unique():
        ...
        if test_row['p_fdr'] < 0.05:
            ...
            print(f"  → No significant differe