...
def load_all_session_results():
    ...
    *_score_changes.csv found under OUTPUT_ROOT/{mouse}/whisker_*/dyad/."""
    ...
    if files:
        ...
        print(f"Loaded {len(files)} per-session CSVs: {len(df):,} rows, "
...
def load_connectivity_summary():
    ...
    if files:
        df = pd.concat([pd.read_csv(f) for f in files], ignore_index=True)
        print(f"Loaded {len(files)} per-session connectivity CSVs: {len(df):,} rows")
        try:
...
def load_session_firing_stats(df):
    ...
    This reads one file per session directly from DATA_ROOT and can take a
    while across many sessions -- ask if you want this parallelized."""
    sessions = df[["mouse", "session_id", "reward_group"]].drop_duplicates()
    rows = []

    for _, row in sessions.iterrows():
        pkl_path = DATA_ROOT / row.session_id / "pairs_light.pkl"
        if not pkl_path.exists():
            warnings.warn(f"pairs_light.pkl not found for {row.session_id}, skipping")
            continue
        ...
        if missing:
            warnings.warn(f"{row.session_id}: pairs_light.pkl missing {missing}, skipping")
            continue
...
def main():
    ...
    print("Within-area vs across-area connectivity...")
    plot_within_vs_across_area(df, SUMMARY_DIR)

    print("Connected pairs by area, per mouse...")
    plot_connected_pairs_by_mouse(df, SUMMARY_DIR)

    print("Connectivity composition by area, pooled + split by reward group...")
    plot_connectivity_by_area_and_reward_group(df, SUMMARY_DIR, stats_dir)

    print("TRUE connectivity rate by reward group (global / within / across)...")
    try:
        connectivity_df = load_connectivity_summary()
        connectivity_df = _filter_excluded_mice(connectivity_df, excluded_mice)
        plot_connectivity_by_reward_group(connectivity_df, SUMMARY_DIR, stats_dir)
