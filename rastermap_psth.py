...
COND_COLORS        = ["#d4a017", "#9e5a00", "#1a55b0", "#005fa0"]
...
def _neuron_vector(st, mouse_id, session_id, event_map, bins, t_ctr, base_mask, cfg):
    ...
    def _mean_psth(trial_type, context):
        ...
        return _causal_gaussian(rates.mean(axis=0), cfg["sigma_ms"] / cfg["bin_ms"])
...
def fit_rastermap(X, n_clusters):
    n_pcs  = min(128, X.shape[0] - 1, X.shape[1] - 1)
...
def _draw_matrix(ax, mat, n_bins, boundaries, vmax, title):
    n, n_total = mat.shape
...
def fig5_population_matrix(X, n_bins, isort, boundaries, vmax, out_dir):
    fig, axes = plt.subplots(1, 2, figsize=(16, 9), dpi=200)
    im1 = _draw_matrix(axes[0], X,        n_bins, [],         vmax, f"Input order  (n={len(X)})")
    im2 = _draw_matrix(axes[1], X[isort], n_bins, boundaries, vmax, "Rastermap order")
...
def run_rastermap_psth(units: pd.DataFrame,
    ...
    units_good = units[units.bc_label == "good"]
    ...
    np.save(out_root / "feature_matrix.npy", X)
    ...
    fig5_population_matrix(X, n_bins, isort, boundaries, vmax, out_root)
    fig6_cluster_profiles(X, t_ctr, n_bins, isort, boundaries, out_root)

    n_pca = min(50, len(unit_ids) - 1, X.shape[1] - 1)
    print(f"Running PCA ({n_pca} components) + UMAP...")
    X_pca = PCA(n_components=n_pca).fit_transform(X)
    fig7_pca_variance(X, n_pca, out_root)
    emb = umap.UMAP(n_neighbors=cfg["umap_n_neighbors"], min_dist=cfg["umap_min_dist"],
                    n_components=2, random_state=42).fit_transform(X_pca)
