...
def _neuron_vector_strided(st, mouse_id, session_id, event_map, t_ctr, base_mask, cfg, conds,
    ...
    #   passive, 4 conds → [wh_norm, au_norm]
    ...
    def _group_norm(vecs):
        """(mean, std) from concatenated vecs; baseline-only if not zscore_full."""
        v = np.concatenate(vecs) if cfg["zscore_full"] else np.concatenate([p[base_mask] for p in vecs])
        return v.mean(), v.std() + 1e-9

    if cfg.get("period", "passive") == "passive":
        if norm_params is None:
            if len(psths) == 4:
                norms = [_group_norm([psths[0], psths[1]]),
                         _group_norm([psths[2], psths[3]])]
            else:
                norms = [_group_norm([psths[0], psths[1]])]
        else:
            norms = norm_params
...
def build_feature_matrix_strided(unit_ids, st_map, mouse_map, session_map, event_map, cfg,
    ...
    """Build (n_neurons, n_conds*n_bins) z-scored PSTH matrix.

    norm_params_list : list of per-neuron norm tuples fitted on another split (e.g. odd trials).
                       When None, norms are computed from the data.
    return_norms     : if True, also return the list of per-neuron norms (for cross-validation).
    """
    dt_stride = cfg["stride_ms"] / 1000
    dt_bin    = cfg["bin_ms"]    / 1000
    ...
    if return_norms:
        ...
        return np.vstack(rows), t_ctr, n_out, norms_list
    return np.vstack(results), t_ctr, n_out
...
def figCV_rastermap_comparison(X_odd, X_even, n_bins, isort, boundaries,
                               vmax, cfg, out_dir):
    """Two-panel figure: X_odd[isort] | X_even[isort]."""
    fig, axes = plt.subplots(1, 3, figsize=(22, 9), dpi=300,
                             gridspec_kw={"width_ratios": [10, 10, 1]})
    im1 = _draw_matrix(axes[0], X_odd[isort],  n_bins, boundaries, vmax, cfg,
                       f"Odd trials — rastermap order (n={len(X_odd)})")
    im2 = _draw_matrix(axes[1], X_even[isort], n_bins, boundaries, vmax, cfg,
...
def figCV_similarity_metrics(X_odd, X_even, isort, boundaries, cluster_labels,
                             n_bins, t_ctr, cond_labels, cond_colors, out_dir):
    """
    Four-panel figure:
...
def run_rastermap_psth(units: pd.DataFrame,
    ...
    out_folder = Path(out_root, "rastermap_psth", f"n_clusters_{n_rastermap_clusters}", period_tag, zscore_tag, mod_tag, reward_tag)
    ...
    print(f"Building feature matrix ({len(unit_ids)} neurons × Z conditions)...")
    ...
    if cfg.get("cross_validate", False):
        ...
        # drop the same odd rows that were flagged in the main pipeline
        # (unit_ids was already pruned of odd/nan/constant rows above)
        ...
        figCV_rastermap_comparison(X_odd, X_even, n_bins, isort_cv, boundaries_cv,
